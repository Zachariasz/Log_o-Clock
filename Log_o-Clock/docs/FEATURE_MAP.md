...
| Reports | `MainWindow` report filters, projections, charts, saved view | `SqliteTrackerStore.GetReportAsync`, `GetSoftwareUsageReportAsync`, `ReportRow`, `DisplayModels`, `CsvExporter` | Current-month default, collapsed project task groups, one filtered Projects/Clients/Software donut with the temporary `Stats for the Boss` idle-inclusive switch, exact/new and equally split legacy software time, saved view, and call-time metrics. |
...
| Client/project management | `MainWindow` clients/projects handlers and context menus | `NewProjectWindow`, `ProjectSettingsWindow`, `ProjectColorWindow`, bulk editor | Double-click edit; blank-space RMB add; projects can be reversibly frozen from the Projects context menu. Frozen project-owned records move to shared, collapsed `Frozen projects` sections across management and Automation; tab changes fold every section, while direct navigation to a frozen project may open its Projects section. Project/client removal is destructive. |
...
| Unified Automation management | `MainWindow` Automation master/detail handlers, progressive `SoftwareSettingsWindow` | `AppController` software observation, software interval/report store methods, `RecognitionEngine`, `AutomationLaunchTrackingSettings` | One project filter/search; application behavior/tags/rule count/history master list; process-specific plus synthetic Any application detail; one editor for app and focused rule modes; Advanced multi-phrase/Any application section; Needs attention; frozen/history-only sections; normal and narrow horizontal access; track launch time option. |
| Self-learning automation | contextual onboarding/title/learned notice in `MainWindow` | pure `AutomationLearningPolicy`; `AppController` 60-second intent and transition; transactional `LearnAutomationAsync`/`UndoLearnedAutomationAsync` | Opt-in and manual-action-only; safe project/task phrase extraction; raw titles memory-only; process-plus-title rules; conflict/exclusion review; 30-second Edit/Undo; history-preserving undo. |
...
| Google Sheets | `GoogleSheetsApiClient`, `GoogleSheetsSyncService`, `SqliteTrackerStore.Sync.cs`, [protocol guide](GOOGLE_SHEETS_SYNC.md) | `GoogleAuthorizationBroker`, pairing/conflict Settings UI, tray/device status, API and multi-store tests | Per profile; hidden append-only journal; create/join; schema 29/protocol 3; one-minute heartbeat; background two-way reconciliation; durable conflicts/deletes; daily tabs are app-managed views. |
...
- Automation: configured applications and recognition rules in one project-centred master-detail view, with shared search/filter, learning, capture, attention summaries, and advanced bulk rule editing.

### Reports

- Date/filter toolbars.
- Project groups with nested task summaries and aligned totals.
- One filtered Projects/Clients/Software donut with the shared `Stats for the Boss` idle-inclusive switch.
...
- Recognition, track all time when computer is launched, the automatic-mode stop/switch grace period, recent-entry continuation, and break reminders (net active-time interval, bottom-right/centre placement, and per-message enablement). The automatic-mode enable switch itself remains beside the title-bar profile selector.
