...
| Reports | `MainWindow` report filters, projections, charts, saved view | `SqliteTrackerStore.GetReportAsync`, `ReportRow`, `DisplayModels`, `CsvExporter` | Current-month default, subtly separated collapsed project task groups with visible column names/totals, latest-task sorting, one selected object, saved visibility/width, active, call-time, and time-plus-idle metrics. |
...
| Google Sheets | `GoogleSheetsApiClient`, `GoogleSheetsSyncService`, `SqliteTrackerStore.Sync.cs`, [protocol guide](GOOGLE_SHEETS_SYNC.md) | `GoogleAuthorizationBroker`, pairing/conflict Settings UI, tray/device status, API and multi-store tests | Per profile; hidden append-only journal; create/join; schema 28/protocol 2; one-minute heartbeat; background two-way reconciliation; durable conflicts/deletes; daily tabs are app-managed views. |
...
- Project/client/software share switchable donut and second time-plus-idle donut.
