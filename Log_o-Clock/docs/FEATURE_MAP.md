...
| Software association and duration | `MainWindow` Software handlers and Reports chart, `SoftwareSettingsWindow` | `AppController` software observation, software interval/report store methods | Software is manual only; exact foreground intervals are recorded only for configured non-excluded software while a timer runs; legacy associations are not duration-backfilled. |
| Window rules | `MainWindow` Rules handlers, `RuleDialog` | `ForegroundActivityMonitor`, `RecognitionEngine` | Group/filter by project; Title contains and Application share the usable default grid width; double-click/RMB edit; delayed process/title capture. |
