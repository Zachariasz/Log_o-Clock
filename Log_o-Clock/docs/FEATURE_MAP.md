...
| Reports | `MainWindow` report filters, projections, charts, saved view | `SqliteTrackerStore.GetReportAsync`, `ReportRow`, `DisplayModels`, `CsvExporter` | Current-month default, collapsed project task groups with visible column names/totals, latest-task sorting, one selected object, saved visibility/width, active, call-time, and time-plus-idle metrics. |
