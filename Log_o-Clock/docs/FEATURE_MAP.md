...
| Unified Automation management | `MainWindow` Automation master/detail handlers, progressive `SoftwareSettingsWindow` | `AppController` software observation, software interval/report store methods, `RecognitionEngine` | One project filter/search; application behavior/tags/rule count/history master list; process-specific plus synthetic Any application detail; one editor for app and focused rule modes; Advanced multi-phrase/Any application section; Needs attention; frozen/history-only sections; normal and narrow horizontal access. |
