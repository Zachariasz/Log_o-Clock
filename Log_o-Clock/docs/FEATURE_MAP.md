...
| Tray | `TrayIconService.cs`, tray handlers in `App.xaml.cs` | `EntryDetailsWindow`, WPF tray smoke flags | Idle/running icons are generated separately from the app icon; single/double click uses Windows double-click timing. |
...
| History presentation | `MainWindow` History refresh/filter/view state | `DisplayModels`, `HistoryGroupDurationConverter`, `TimeEntryOverlapDetector`, wrapping converter | Group by local day, daily total, passive selection, description/tag filters, overlap icon, saved columns/order/width/wrap. |
| Reports | `MainWindow` report filters, projections, charts, saved view | `SqliteTrackerStore.GetReportAsync`, `ReportRow`, `DisplayModels`, `CsvExporter` | Current-month default, project groups, latest-task sorting, one selected object, saved visibility/width, active and time-plus-idle metrics. |
...
| Client/project management | `MainWindow` clients/projects handlers and context menus | `NewProjectWindow`, `ProjectSettingsWindow`, `ProjectColorWindow`, bulk editor | Double-click edit; blank-space RMB add; project/client removal is destructive. |
| Task management | `MainWindow` Tasks handlers | `NewTaskWindow`, `BulkEditWindow`, Trello link records | Project filter resets on entry; linked Trello tasks cannot be locally renamed/moved. |
...
| Recognition reminders | `AppController` recognition methods | `NotificationService`, `ReminderWindow`, `ProjectChooserWindow`, `TaskTitleMatcher`, recognition tests | 500 ms stability, longest rule, ambiguity chooser, matched-monitor placement, task suggestion, five-minute snooze. |
...
| Google Sheets | `GoogleSheetsApiClient`, `GoogleSheetsSyncService` | `GoogleAuthorizationBroker`, Settings UI, credential store, API tests | Per profile; OAuth refresh token in Credential Manager; daily worksheets; queued 5 s plus 15-minute sync; export only. |
...
- Trello connection/mappings and Google Sheets export connection.
