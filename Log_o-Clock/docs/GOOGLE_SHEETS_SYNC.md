...
Every computer sharing a spreadsheet must run a schema-29, synchronization-protocol-3 compatible release. An older export-only client must not write to an upgraded spreadsheet.

## Shared and device-local data

The shared profile includes:

- Completed time entries, including raw UTC start/end values, project/task identity, description, pending/source state, paid/call state, stable creation time, modification time, exact exclusions, associated configured software, exact configured-software foreground intervals, and legacy equal-allocation boundaries.
