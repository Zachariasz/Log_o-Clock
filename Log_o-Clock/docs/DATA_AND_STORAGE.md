...
Launch-time tracking also requires no schema migration: `automation.track-launch-time.enabled` defaults to `false` in the `Settings` table and is a profile-scoped stable setting eligible for whole-profile synchronization. When enabled, Log O'clock automatically starts an unassigned time entry silently whenever the app is initialized and no timer is running, as well as on session unlock and resume.

Automation learning also requires no schema or synchronization-protocol migration. It reuses `Software`, `ProjectSoftwareSettings`, and `RecognitionRules`; `automation.learning.enabled` and `automation.learning.onboarding-seen` are profile-scoped stable settings eligible for whole-profile synchronization. The 60-second learning intent, unknown observed title, pending phrase review, and 30-second Undo/Edit notice remain runtime-only.

`LearnAutomationAsync` is idempotent and transactional. It reuses the case-insensitive software process identity, preserves an existing global scope, safely adds a missing project assignment, never replaces an exclusion or restores removed software, and creates only process-plus-title rules. Active rules that could match the observed title in another project return a conflict instead of adding ambiguity. Its result records exactly which project setting and rule were created. `UndoLearnedAutomationAsync` deletes only those records; it does not remove the software identity, entry-software association, historical label, or exact software interval. No unknown/raw title is accepted as an implicit persisted value.
