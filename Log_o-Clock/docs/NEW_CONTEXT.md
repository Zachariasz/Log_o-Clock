...
- Persistence: per-profile SQLite, current schema **29**.
...
- Google Sheets requires every participating computer to run the schema-29/protocol-3 version. Legacy visible-only rows are archived and offered as explicit Import/Ignore candidates.
